...
def parse_args():
    parser = argparse.ArgumentParser(description="Optuna tuner for standalone DQN.")
    parser.add_argument("--study-name", type=str, default="dqn_optuna")
...
def main():
    ...
    storage = f"sqlite:///{os.path.join(study_root, 'optuna_study_dqn.db')}"
    ...
    best_path = os.path.join(study_root, "best_params_dqn.json")
    with open(best_path, "w") as f:
        json.dump(best, f, indent=2)

    trials_csv_path = os.path.join(study_root, "trials_dqn.csv")
