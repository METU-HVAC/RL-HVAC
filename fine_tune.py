...
def parse_args():
    ...
    p.add_argument("--project", default=os.environ.get("WANDB_PROJECT", "A403-FineTune"))
...
def main():
    ...
    fine_tune_experiment_name = create_experiment_name(env_name=ENV_NAME, episodes=1, algorithm_name=ALGORITHM_NAME)
    experiment_save_dir_name = os.path.join(run_dir, "results", args.algorithm, fine_tune_experiment_name)
    os.makedirs(experiment_save_dir_name, exist_ok=True)

    print(f"FineTuning model: {args.model_path}")
