...
observation_variables = [
    ...
    'outdoor_temperature_future_1','people_occupant_future_1',
    'outdoor_temperature_future_2','people_occupant_future_2'
]
...
future_csv_path = './logs/whole_year_2.csv'
n_future = 2
predictor = FutureObservationProvider(future_csv_path, n=n_future)
...
def augment_state_with_future(state_array, env, predictor, current_step, timesteps_per_hour, device):
    obs_variables = env.get_wrapper_attr('observation_variables')
    state_dict = dict(zip(obs_variables, state_array))

    month = int(state_dict['month'])
    day = int(state_dict['day_of_month'])
    hour = int(state_dict['hour'])
    minute = int(((current_step - 1) % timesteps_per_hour) * (60 / timesteps_per_hour))
    time_label = f"{month:02}-{day:02} {hour:02}:{minute:02}"

    future_obs = predictor.get_future_observations(time_label)

    # Debug Print Start
    # print(f"\n--- Step {current_step} ---")
    # print(f"Time Label: {time_label}")
    # print(f"Current | Temp: {state_dict['outdoor_temperature']:.2f}  People: {state_dict['people_occupant']:.1f}")
    
    # for i, obs in enumerate(future_obs, 1):
    #     print(f"Future t+{i} | Temp: {obs['outdoor_temperature']:.2f}  People: {obs['people_occupant']:.1f}")
    # Debug Print End

    # Prepare appended features
    appended_features = []
    ...
    appended_features_tensor = torch.tensor(appended_features, dtype=torch.float32, device=device).unsqueeze(0)
    augmented_state = torch.cat([state_tensor, appended_features_tensor], dim=1)
...
def run_simulation(env_id,start_date, end_date, season,episode_type, steps_per_chunk,fan_agent,ac_agent,train_interval,timesteps_per_hour,reward_config):
    ...
    while current_step < steps_per_chunk:
        ...
        # Augment next_state
        augmented_next_state = augment_state_with_future(next_state, env, predictor, current_step+1, timesteps_per_hour, device)
        normalized_next_state = torch.tensor(min_max_normalize(augmented_next_state, obs_mins, obs_maxs),
                                             dtype=torch.float32, device=device)
