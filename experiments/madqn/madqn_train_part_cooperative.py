...
def get_agent_observation_dict_based(agent_name: str, observation: List[float], action: int) -> List[float]:
    ...
    if agent_name == "WindowFan":
        ...
        keys = ['hour','outdoor_temperature','air_temperature', 'air_humidity','people_occupant', 'window_fan_speed','weekday', 'total_electricity_HVAC','pmv','ppd']
...
def train(config=None):
    with wandb.init(config=config):
        ...
        ac_agent = DQNAgent(10, 10,total_training_steps,num_episodes,training_config)
        if config.fine_tune:
