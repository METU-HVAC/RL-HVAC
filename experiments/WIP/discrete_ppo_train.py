...
from utils.discretization import discretize_actions,scale_actions
from algorithms.ppo.old_ppo import *
...
def run_simulation(start_date, end_date, episode_type, steps_per_chunk,agent,train_interval,timesteps_per_hour,reward_config):
    env = create_environment(start_date, end_date,CO2andTemperatureReward,env_name='Eplus-A403-hot-discrete-v1',timesteps_per_hour=timesteps_per_hour,reward_kwargs=reward_config)  # Create a new environment for the chunk
    ...
    while current_step < steps_per_chunk:
        ...
        if episode_type == "Training":
            ...
            action, prob, val = agent.choose_action(normalized_obs)
        else:
            action, prob, val = agent.choose_action(normalized_obs,deterministic=True)
        ...
        normalized_next_state = normalize_observation(next_state,obs_means,obs_stds)
            #next_state = normalize_observation(next_state,obs_mean,obs_std_dev)
        if episode_type == "Training":
            # Store transition in replay buffer
            agent.remember(normalized_next_state, action, prob, val, reward, done)
...
def train(config=None):
    with wandb.init(config=config):
        ...
        days_per_chunk = 10
        total_days = 365
        plots_dir = "results/plots/ppo"  # Directory to store plots
        ...
        for episode in range(1, num_episodes + 1):
            with tqdm(total=len(train_chunks) + len(val_chunks), 
                ...
                with open("val_average_reward.txt", "a") as f:
                    f.write(f"Episode {episode}: val Average Reward = {avg_val_reward}\n")
        ...
        file_path = os.path.join(experiment_dir, "ppo_data.csv")
