...
def get_agent_observation_dict_based(agent_name: str, observation: List[float], action: int) -> List[float]:
    ...
    if agent_name == "WindowFan":
        ...
        keys = ['hour','outdoor_temperature', 'outdoor_humidity','air_temperature', 'people_occupant', 'window_fan_speed','weekday', 'total_electricity_HVAC','pmv','ppd']
...
def train(config=None):
    with wandb.init(config=config):
        ...
        fan_agent = SACDiscrete(obs_dim=5, action_dim=4, updates_per_step=4, 
        ...
        ac_agent = SACDiscrete(obs_dim=9, action_dim=10, updates_per_step=4, 
                            buffer_size=memory_capacity, learning_rate=learning_rate, batch_size=64, device=device,num_of_episodes=num_episodes).to(device)
