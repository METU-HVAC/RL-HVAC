...
def run_simulation(env_id,start_date, end_date, season,episode_type, steps_per_chunk,fan_agent,ac_agent,train_interval,timesteps_per_hour,reward_config,switching_penalty ):
    ...
    while current_step < steps_per_chunk:
        ...
        if episode_type == "Training":
            ...
            fan_action = fan_agent.choose_action(fan_obs_tensor, greedy=True)
            hvac_action = ac_agent.choose_action(hvac_obs_tensor, greedy=True)
            
        else:
            #action = agent.choose_greedy_action(normalized_reduced_state)  # Greedy action for validation/testing
            fan_action = fan_agent.choose_action(fan_obs_tensor)
            hvac_action = ac_agent.choose_action(hvac_obs_tensor)
        ...
        fan_reward = torch.tensor([get_agent_reward("CombinedAgent", info)], dtype=torch.float32, device=device)
        hvac_reward = torch.tensor([get_agent_reward("CombinedAgent", info)], dtype=torch.float32, device=device)
        ...
        if episode_type == "Training":
            ...
            fan_agent.replay_buffer.store(fan_obs_tensor, fan_action.cpu().numpy(),  fan_reward.cpu().numpy(),next_fan_obs_tensor,done)
            ac_agent.replay_buffer.store(hvac_obs_tensor, hvac_action.cpu().numpy(),  hvac_reward.cpu().numpy(),next_hvac_obs_tensor,done)
            # Train DQN every few steps if buffer size is sufficient
            if current_step % train_interval == 0 and fan_agent.replay_buffer.size >= fan_agent.batch_size and ac_agent.replay_buffer.size >= ac_agent.batch_size:
                fan_losses = []
                hvac_losses = []
                fan_actor_loss, fan_critic_loss, fan_alpha_loss = fan_agent.update()
                fan_total_loss = fan_actor_loss + fan_critic_loss + fan_alpha_loss
                hvac_actor_loss, hvac_critic_loss, hvac_alpha_loss = ac_agent.update()
                hvac_total_loss = hvac_actor_loss + hvac_critic_loss + hvac_alpha_loss
                if fan_total_loss is not None:
                    fan_losses.append(fan_total_loss)
                if hvac_total_loss is not None:
                    hvac_losses.append(hvac_total_loss)
...
def train(config=None):
    with wandb.init(config=config):
        ...
        fan_agent = SACDiscrete(obs_dim=12, action_dim=4, updates_per_step=4, 
                            buffer_size=memory_capacity, learning_rate=learning_rate, batch_size=64, device=device,num_of_episodes=num_episodes).to(device)
        ac_agent = SACDiscrete(obs_dim=12, action_dim=10, updates_per_step=4, 
                            buffer_size=memory_capacity, learning_rate=learning_rate, batch_size=64, device=device,num_of_episodes=num_episodes).to(device)
