#!/usr/bin/env python3
"""
Standalone evaluation script for MADQN HVAC + Fan control.
...
def run_evaluation(
    ...
    while current_step < steps_per_chunk:
        ...
        total_hvac_reward += info["comfort_term"] + info["ac_energy_term"]
...
def generate_plots(obs_dict, output_dir, season):
    """Generate evaluation plots for MADQN (temperature-based comfort)."""
    import matplotlib.pyplot as plt

    plt.style.use("seaborn-v0_8-darkgrid")
    fig, axes = plt.subplots(3, 2, figsize=(14, 12))
    fig.suptitle(f"MADQN Evaluation - {season.upper()} Season", fontsize=14, fontweight="bold")
    ...
    # 2) Temperature deviation (comfort proxy)
    ax2 = axes[0, 1]
    if "temp_deviations" in obs_dict:
        deviations = np.array([0.0 if v is None else v for v in obs_dict["temp_deviations"]])
        ax2.plot(timesteps, deviations, color="#9b59b6", linewidth=1)
        ax2.axhline(y=0, color="green", linestyle="--", alpha=0.5, label="No Violation")
        ax2.set_ylabel("Temp Deviation (C)")
        ax2.set_title("Temperature Comfort Deviation")
    ...
    # 6) MADQN-specific split control trajectories
    ax6 = axes[2, 1]
...
def create_summary_plot(obs_dict, output_dir, season):
    """Create a compact summary plot for quick visual comparison."""
    import matplotlib.pyplot as plt
...
def main():
    parser = argparse.ArgumentParser(description="Standalone MADQN Evaluation (No WandB)")
    ...
    with tqdm(val_chunks, desc="Evaluating", ncols=100) as pbar:
        for chunk in pbar:
            ...
            if "temp_violations" in obs_dict:
                vals = [v for v in obs_dict["temp_violations"] if v is not None]
                if vals:
                    temp_violations.append(sum(vals) / len(vals) * 100)
    ...
    results = {
        ...
        "temp_violation_pct": float(np.mean(temp_violations)) if temp_violations else None,
    ...
    # Persist a compact final trajectory to inspect behavior quickly.
    if last_obs_dict:
