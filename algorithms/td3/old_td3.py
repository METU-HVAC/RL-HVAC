...
class OUActionNoise(object):
    def __init__(self, mu, sigma=0.9, theta=.1, dt=1e-2, x0=None):
        self.theta = theta
        self.mu = mu
        self.sigma = sigma
        self.dt = dt
...
class Agent():
    def __init__(self, alpha, beta, input_dims, tau, min_action=-1,max_action=1,
    ...
    def choose_action(self, observation):
        if self.time_step < self.warmup:
           # Generate random action within bounds
            mu = T.tensor(
                np.random.uniform(self.min_action, self.max_action, size=(self.n_actions,))
